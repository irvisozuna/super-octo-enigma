      ...
      <PerfectScrollbar tag="ul" :options="{ wheelPropagation: false }">
        ...
        <CustomizerSection title="Theming" :divider="false">
          ...
          <div class="d-flex flex-column gap-2">
            ...
            <div class="d-flex app-customizer-secondary-colors" style="column-gap: 0.75rem; margin-block-start: 2px;">
              <div v-for="color in colors" :key="color.main" style="
              border-radius: 0.375rem;
              outline: 1px solid rgba(var(--v-border-color), var(--v-border-opacity));
              padding-block: 0.5rem;
              padding-inline: 0.625rem;" class="secondary-color-wrapper cursor-pointer"
                :class="vuetifyTheme.current.value.colors.secondary === color.main ? 'active' : ''"
                :style="vuetifyTheme.current.value.colors.secondary === color.main ? `outline-color: ${color.main}; outline-width:2px;` : `--v-color:${color.main}`"
                @click="setSecondaryColor(color)">
                <div style="border-radius: 0.375rem;block-size: 2.125rem; inline-size: 1.8938rem;"
                  :style="{ backgroundColor: color.main }" />
              </div>

              <div class="secondary-color-wrapper cursor-pointer d-flex align-center" style="
              border-radius: 0.375rem;
              outline: 1px solid rgba(var(--v-border-color), var(--v-border-opacity));
              padding-block: 0.5rem;
              padding-inline: 0.625rem;"
                :class="vuetifyTheme.current.value.colors.secondary === customSecondaryColor ? 'active' : ''"
                :style="vuetifyTheme.current.value.colors.secondary === customSecondaryColor ? `outline-color: ${customSecondaryColor}; outline-width:2px;` : ''">
                <VBtn icon size="30"
                  :color="vuetifyTheme.current.value.colors.secondary === customSecondaryColor ? customSecondaryColor : $vuetify.theme.current.dark ? '#8692d029' : '#4b465c29'"
                  variant="flat" style="border-radius: 0.375rem;">
                  <VIcon size="20" icon="tabler-color-picker"
                    :color="vuetifyTheme.current.value.colors.secondary === customSecondaryColor ? 'rgb(var(--v-theme-on-secondary))' : ''" />
                </VBtn>

                <VMenu activator="parent" :close-on-content-click="false">
                  <VList>
                    <VListItem>
                      <VColorPicker v-model="customSecondaryColor" mode="hex" :modes="['hex']"
                        @update:model-value="setSecondaryColor({ main: customSecondaryColor, darken: customSecondaryColor })" />
                    </VListItem>
                  </VList>
                </VMenu>
              </div>
