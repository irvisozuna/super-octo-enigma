...
const totalCourse = 10//computed(() => courseData.value.total)
</script>
  ...
  <VCard>
    <VCardText>
      <div class="d-flex flex-wrap justify-space-between align-center gap-4">
        ...
        <div>
          <AppTextField v-model="searchQuery" placeholder="Search Course"
    ...
    <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :items-per-page-options="[
    ...
    ]" :headers="headers" :items="[]" :items-length="10" show-select class="text-no-wrap" @update:options="updateOptions">
      <!-- Course Name -->
        ...
        <div class="d-flex align-center gap-x-4 py-2">
          ...
          <div>
            <div class="text-base font-weight-medium mb-1">
              <RouterLink :to="{ name: 'apps-academy-course-details' }" class="text-link d-inline-block">
                {{ item.courseTitle }}
              </RouterLink>
            </div>
      ...
      <template #item.progress="{ item }">
        <div class="d-flex align-center gap-x-4" style="inline-size: 15.625rem;">
          ...
          <div class="w-100">
            <VProgressLinear color="primary" height="8"
              :model-value="Math.floor((item.completedTasks / item.totalTasks) * 100)" rounded />
          </div>
      ...
      <!-- Pagination -->
      <template #bottom>
        <TablePagination v-model:page="page" :items-per-page="itemsPerPage" :total-items="totalCourse" />
      </template>
    </VDataTableServer>
  </VCard>
</template>
