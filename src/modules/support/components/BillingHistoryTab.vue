...
async function fetchData() {
  loading.value = true
  try {
    const response = await contractStore.getBillingsByContract(contractStore.item.id_account, pagination.value)
      ...
      >
        <template #FECHA_FACTURACION="{ item }">
          <div class="text-center">
            {{ item.FECHA_FACTURACION ? new Date(`${item.FECHA_FACTURACION}T00:00:00`).toLocaleDateString('es-ES') : 'N/A' }}
          </div>
        </template>
        <template #MONTO="{ item }">
          <div class="text-end">
