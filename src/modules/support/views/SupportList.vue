...
<template>
  <VCard>
    ...
    <SupportFilters
      :initial-filters="supportStore.filters"
    ...
    <VCardText>
      <div class="d-flex align-center flex-wrap gap-4 justify-end">
        <!-- Campo de búsqueda -->
        <VTextField
          v-model="supportStore.filters.search"
          :label="$t('Search Support')"
          variant="outlined"
          dense
          class="filter-field"
          @input="applyFilters"
        ...
        <VBtn
          v-if="supportStore.selectedItems.length > 1"
          color="error"
    ...
    <SupportTable
      :headers="headers"
      :items="supportStore.list"
      :total="supportStore.total"
      :page="supportStore.page"
      :items-per-page="supportStore.itemsPerPage"
      :loading="supportStore.isLoading"
      :selection="supportStore.selectedItems"
      @update:selection="val => supportStore.selectedItems = val"
