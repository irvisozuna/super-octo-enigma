  ...
  <VCard class="pa-sm-10 pa-2">
    <VCardText>
      ...
      <h4 class="text-h4 text-center mb-2">
        {{ t('editModule', { moduleName: supportTitle }) }}
      </h4>
...
async function onFormSubmit(values: any) {
  ...
  try {
    ...
    // Si tu API necesita supportId, asegúrate de tenerlo en props.item
    if (!props.item.id) {
      console.warn('No ID found in props.item, cannot update.');
      return;
    }

    await supportStore.updateItem(props.item.id, values);
    close('submit');
  } catch (error) {
