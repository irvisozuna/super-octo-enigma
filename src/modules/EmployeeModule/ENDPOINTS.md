...
```json
{
  "reason": "Violación de políticas de la empresa",
  "notes": "Suspensión temporal por 15 días mientras se realiza la investigación correspondiente"
}
```

**Field Validations:**
- `reason` (optional, string, max:500) - Reason for suspension
- `notes` (optional, string, max:1000) - Additional notes
...
curl -X POST 'http://localhost/api/employees/{employee_id}/suspend' \
  ...
  -d '{
    "reason": "Violación de políticas de la empresa",
    "notes": "Suspensión temporal por 15 días"
  }'
```
...
```json
{
  "reason": "Cumplió con el periodo de suspensión",
  "notes": "Empleado completó satisfactoriamente el periodo de suspensión y capacitación requerida"
}
```

**Field Validations:**
- `reason` (optional, string, max:500) - Reason for reactivation
- `notes` (optional, string, max:1000) - Additional notes
...
curl -X POST 'http://localhost/api/employees/{employee_id}/reactivate' \
  ...
  -d '{
    "reason": "Cumplió con el periodo de suspensión",
    "notes": "Empleado completó satisfactoriamente el periodo de suspensión"
  }'
```
...
```json
{
  ...
  "notes": "El contrato temporal de 6 meses finalizó de acuerdo a lo establecido. Empleado cumplió satisfactoriamente con sus obligaciones"
}
```

**Field Validations:**
- `reason` (optional, string, max:500) - Reason for termination
- `notes` (optional, string, max:1000) - Additional notes
...
curl -X POST 'http://localhost/api/employees/{employee_id}/terminate' \
  ...
  -d '{
    "reason": "Fin de contrato temporal",
    "notes": "El contrato temporal de 6 meses finalizó de acuerdo a lo establecido"
  }'
```
...
### 13. Get Employee Skills
**GET** `/api/employees/{id}/skills`

Get all skills for a specific employee, ordered by proficiency level and years of experience.
...
**Example Request:**
```bash
curl -X GET 'http://localhost/api/employees/{employee_id}/skills' \
  -H 'Authorization: Bearer {token}' \
  -H 'X-Company-Id: {company_id}' \
  -H 'Accept: application/json'
```
  ...
  "data": [
    {
      ...
      "notes": "Certificado en transporte de carga peligrosa",
      "is_advanced": true,
      "can_train_others": true,
      "experience_score": 60,
      "created_at": "2025-10-12T00:00:00.000000Z",
      "updated_at": "2025-10-12T00:00:00.000000Z"
    }
  ]
}
```
...
### 17. Get Employee Certifications
**GET** `/api/employees/{id}/certifications`

Get all certifications for a specific employee, ordered by issue date (most recent first).
...
**Example Request:**
```bash
curl -X GET 'http://localhost/api/employees/{employee_id}/certifications' \
  -H 'Authorization: Bearer {token}' \
  -H 'X-Company-Id: {company_id}' \
  -H 'Accept: application/json'
```
...
### 18. Add Certification to Employee
...
4. **Status Change Parameters**: All status change endpoints (suspend, terminate, reactivate) accept optional `reason` and `notes` parameters that are stored in the employment history
5. **Soft Deletes**: Skills and Certifications use soft deletes and can be restored
6. **UUID Primary Keys**: All resources use UUID primary keys
7. **Pagination**: List endpoints support pagination via `per_page` query parameter
8. **Validation**: All requests are validated with Spanish error messages
