<template>
  <VCard class="pa-sm-10 pa-2">
    <VCardText>
      <!-- Título -->
      <h4 class="text-h4 text-center mb-2">{{ t('addModule', { moduleName: templateTitle }) }}</h4>
...
const templateTitle = t('template');
const roleOptions = ref([]);
const showPassword = ref(false);

// Obtenemos el schema
const { templateCreateValidation } = useTemplateValidation();

// Configuramos el formulario con Vee Validate
const { handleSubmit, errors } = useForm({
  validationSchema: templateCreateValidation,
  initialValues: {
    first_name: '',
    last_name: '',
    email: '',
...
async function onFormSubmit(values: any) {
  try {
    await templateStore.createItem(values);
    close('submit');
  } catch (error) {
