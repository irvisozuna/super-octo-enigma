...
import { usePaymentmandateValidation } from '@/modules/paymentmandate/composables/usePaymentmandateValidation'
...
const paymentmandateTitle = 'paymentmandate'
const roleOptions = ref<any[]>([])
const showPassword = ref(false)

// Obtenemos el schema para edición
const { paymentmandateEditValidation } = usePaymentmandateValidation()

// Configuramos el formulario con vee-validate
// Cargamos datos iniciales desde props.item
const { handleSubmit, errors } = useForm({
  validationSchema: paymentmandateEditValidation,
  initialValues: {
...
async function onFormSubmit(values: any) {
  ...
  try {
    ...
    // Si tu API necesita paymentmandateId, asegúrate de tenerlo en props.item
    if (!props.item.id) {
    ...
    await paymentmandateStore.updateItem(props.item.id, values)
    close('submit')
  }
  catch (error) {
  ...
  <VCard class="pa-sm-10 pa-2">
    <VCardText>
      ...
      <h4 class="text-h4 text-center mb-2">
        {{ t('editModule', { moduleName: paymentmandateTitle }) }}
      </h4>
