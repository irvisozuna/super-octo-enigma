// usePaymentmandateValidation.ts
import { useI18n } from 'vue-i18n';
import * as yup from 'yup';

export function usePaymentmandateValidation() {
  const { t } = useI18n();
  
  const paymentmandateBaseValidation = yup.object({
  ...
  const paymentmandateCreateValidation = paymentmandateBaseValidation.shape({
    role: yup.array()
      .required(t('validations.required', { field: t('fields.role') }))
      .min(1, t('validations.minItems', { field: t('fields.role'), count: 1 })),
  });

  const paymentmandateEditValidation = paymentmandateBaseValidation.shape({
  ...
  return {
    paymentmandateBaseValidation,
    paymentmandateCreateValidation,
    paymentmandateEditValidation,
  };
}
