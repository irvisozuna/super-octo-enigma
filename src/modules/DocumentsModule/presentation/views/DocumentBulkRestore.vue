...
const loadDocuments = async () => {
  try {
    ...
    // Only load archived documents
    documents.value = await documentStore.fetchArchivedDocuments()
  }
  catch (err) {
    error.value = 'Error al cargar los documentos archivados'
...
const restoreSelected = async () => {
  ...
  if (confirm(t('DocumentsModule.messages.confirm_bulk_restore', { count: selectedCount.value }))) {
    try {
      restoring.value = true

      await documentStore.bulkRestoreDocuments(selectedDocuments.value)
...
<template>
  <div class="document-bulk-restore">
    <!-- Header -->
    <div class="d-flex justify-space-between align-center mb-6">
      <div>
        <h1 class="text-h4 font-weight-bold">
          {{ t('DocumentsModule.document.bulk_restore') }}
        </h1>
        <p class="text-body-1 text-medium-emphasis mt-1">
          {{ t('DocumentsModule.document.bulk_restore_subtitle') }}
        </p>
      </div>
      ...
      <VCardText>
        <div class="d-flex justify-space-between align-center">
          ...
          <div class="d-flex gap-2">
            ...
            >
              {{ t('DocumentsModule.document.restore_selected') }}
            </VBtn>
      ...
      <VCard v-if="documents.length === 0">
        <VCardText class="text-center py-8">
          ...
          <div class="text-body-2 text-medium-emphasis">
            {{ t('DocumentsModule.document.no_archived_documents') }}
          </div>
        </VCardText>
      </VCard>
        ...
        <VCard class="mb-4">
          <VCardText>
            <div class="d-flex align-center gap-3">
              ...
              <div class="text-body-2 text-medium-emphasis">
                {{ documents.length }} {{ t('DocumentsModule.document.total_archived_documents') }}
              </div>
            </div>
          </VCardText>
        </VCard>
