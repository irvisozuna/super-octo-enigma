...
// Form for bulk tagging
const tagForm = ref({
  action: 'add', // add, remove, replace
  tags: '',
  existing_tags: [] as string[],
})
...
const executeTag = async () => {
  ...
  try {
    ...
    const tags = tagForm.value.tags.split(',').map(tag => tag.trim()).filter(tag => tag)

    switch (tagForm.value.action) {
      case 'add':
        await documentStore.bulkAddTags(selectedDocuments.value, tags)
        break
      case 'remove':
        await documentStore.bulkRemoveTags(selectedDocuments.value, tags)
        break
      case 'replace':
        await documentStore.bulkReplaceTags(selectedDocuments.value, tags)
        break
    }
    ...
    console.error('Error tagging documents:', err)
  }
  finally {
    tagging.value = false
  }
...
<template>
  <div class="document-bulk-tag">
    <!-- Header -->
    <div class="d-flex justify-space-between align-center mb-6">
      <div>
        <h1 class="text-h4 font-weight-bold">
          {{ t('DocumentsModule.document.bulk_tag') }}
        </h1>
        <p class="text-body-1 text-medium-emphasis mt-1">
          {{ t('DocumentsModule.document.bulk_tag_subtitle') }}
        </p>
      </div>
        ...
        <div class="d-flex justify-space-between align-center">
          ...
          <div class="d-flex gap-2">
            <VBtn
              ...
              @click="handleTag"
            >
              {{ t('DocumentsModule.document.tag_selected') }}
            </VBtn>
    ...
    >
      <VCardTitle>
        {{ t('DocumentsModule.document.tag_form') }}
      </VCardTitle>

      <VCardText>
        <VForm @submit.prevent="executeTag">
          <VRow>
            <VCol
              cols="12"
              md="6"
            >
              <VSelect
                v-model="tagForm.action"
                :items="[
                  { value: 'add', title: 'Add Tags' },
                  { value: 'remove', title: 'Remove Tags' },
                  { value: 'replace', title: 'Replace All Tags' },
                ]"
                :label="t('DocumentsModule.document.tag_action')"
                variant="outlined"
                required
              />
            </VCol>
            ...
            >
              <VTextField
                ...
                :label="t('DocumentsModule.document.tags')"
                variant="outlined"
