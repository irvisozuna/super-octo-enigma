  ...
  <VCard>
    <VCardText class="text-center pt-12">
      ...
      <VAvatar
        rounded
        :size="100"
        :color="!itemData.profile.avatar_url ? 'primary' : undefined"
        :variant="!itemData.profile.avatar_url ? 'tonal' : undefined"
      >
        <VImg
          v-if="itemData.profile.avatar_url"
          :src="itemData.profile.avatar_url"
        />
        <span
          v-else
      ...
      <h5 class="text-h5 mt-4">
        {{ itemData.profile.first_name }} {{ itemData.profile.last_name }}
      </h5>

      <!-- role -->
      <h6 class="text-subtitle-2 mt-1">
        ({{ itemData.roles.map(role => role.name).join(', ') }})
      </h6>

      <!-- Bio -->
      <p class="mt-4">
        {{ itemData.profile.bio }}
      </p>

      <!-- Role -->
      <VChip
        v-if="itemData.preferences && itemData.preferences.theme_preference"
        label
        :color="resolveUserRoleVariant(itemData.preferences.theme_preference).color"
        size="small"
        class="mt-4"
      >
