<template>
  ...
  <VDataTableServer :headers="headers" :items="items" :items-length="total" :page="localPage"
    :items-per-page="localItemsPerPage" :loading="loading" show-select :items-per-page-options="itemsPerPageOptions"
    @update:page="updatePage" @update:items-per-page="updateItemsPerPage">
    <template #item.id="{ item }">{{ item.id }} </template>
    <template #item.name="{ item }">{{ item.user.name }}</template>
